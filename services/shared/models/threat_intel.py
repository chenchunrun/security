...
class ThreatIntel(BaseModel):
    ...
    threat_level: ThreatLevel = Field(
        ..., description="Threat classification level"
    )
    ...
    first_seen: Optional[datetime] = Field(
        default=None, description="First reported timestamp"
    )
    last_seen: Optional[datetime] = Field(
        default=None, description="Last reported timestamp"
    )
...
class ThreatIntelQuery(BaseModel):
    ...
    sources: Optional[list[IntelSource]] = Field(
        default=None,
        description="Specific sources to query (null = all)"
    )
...
class AggregatedThreatIntel(BaseModel):
    ...
    ioc_value: str = Field(..., description="Indicator value")
    threat_level: ThreatLevel = Field(
        ..., description="Aggregated threat level"
    )
    ...
    positive_sources: int = Field(
        ..., ge=0, description="Number of sources reporting malicious"
    )
    total_sources: int = Field(
        ..., ge=1, description="Total number of sources queried"
    )
